...
class ClientPortTest {
    ...
    void getTestNotThrow() {
        Assertions.assertDoesNotThrow(() -> {
            this.clientPort.get((Specification<ClientEntity>) (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(ClientEntity_.CLIENT_ID), 1001))
    ...
    void getTestThrow() {
        Assertions.assertThrows(PersistenceAdapterException.class, () -> {
            this.clientPort.get((Specification<ClientEntity>) (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(ClientEntity_.CLIENT_ID), 999))
                    .orElseThrow(() -> new PersistenceAdapterException("Client not found"));
        });
    }
    ...
    @Test
    void updateNotThrow() {
        ClientEntity client = this.clientPort.get((Specification<ClientEntity>) (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(ClientEntity_.CLIENT_ID), 1001))
                .orElseThrow(() -> new PersistenceAdapterException("Client not found"));
    ...
    void deleteNotThrow() {
        ...
        ClientEntity deletedClient = this.clientPort.get((Specification<ClientEntity>) (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(ClientEntity_.CLIENT_ID), 1001))
                .orElseThrow(() -> new PersistenceAdapterException("Client not found"));
