...
class TicketPortTest {
    ...
    void getTestNotThrows() {
        Assertions.assertDoesNotThrow(() -> {
            this.ticketPort.get(((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(TicketEntity_.ID).get(TicketIdEntity_.TICKET_ID), 10001)))
    ...
    void getTestThrows() {
        Assertions.assertThrows(PersistenceAdapterException.class, () -> {
            this.ticketPort.get(((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(TicketEntity_.ID).get(TicketIdEntity_.TICKET_ID), 9999)))
                    .orElseThrow(() -> new PersistenceAdapterException("Ticket not found"));
        });
    }
    ...
    void updateTestNotThrows() {
        TicketEntity ticketEntity = this.ticketPort.get(((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(TicketEntity_.ID).get(TicketIdEntity_.TICKET_ID), 10001)))
                .orElseThrow(() -> new PersistenceAdapterException("Ticket not found"));
    ...
    void deletedNotThrows() {
        ...
        TicketEntity deletedTicketEntity = this.ticketPort.get(((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(TicketEntity_.ID).get(TicketIdEntity_.TICKET_ID), 10001)))
                .orElseThrow(() -> new PersistenceAdapterException("Ticket not found"));
