...
public class TicketService implements DomainService<Ticket, TicketEntity> {
  ...
  public TicketService(TicketPort ticketPort, TicketStatusService ticketStatusService,
      UserService userService, ClientService clientService, ModelMapper modelMapper) {
  ...
  public Ticket get(Long id) {
    TicketEntity ticketEntity = this.ticketPort.get(
            ((root, query, criteriaBuilder) -> criteriaBuilder.equal(
                root.get(TicketEntity_.ID).get(TicketIdEntity_.TICKET_ID), id)))
  ...
  public Ticket getByClientId(Long id) {
    TicketEntity ticketEntity = this.ticketPort.get(
            ((root, query, criteriaBuilder) -> criteriaBuilder.equal(
                root.get(TicketEntity_.ID).get(TicketIdEntity_.CLIENT_ID), id)))
        .orElseThrow(() -> TICKET_NOT_FOUND);
    return convertEntityToModel(ticketEntity);
  }
  ...
  public Ticket updateTicket(UpdateTicketRequest updateTicketRequest) {
    TicketEntity ticketEntity = this.ticketPort.get(
        ((root, query, criteriaBuilder) -> criteriaBuilder.equal(
            ...
            updateTicketRequest.getTicketId()))).orElseThrow(() -> TICKET_NOT_FOUND);
    ticketEntity.setTicketStatusId(updateTicketRequest.getTicketStatusId());
    return convertEntityToModel(this.ticketPort.update(ticketEntity));
  }
  ...
  public Page<Ticket> getAll(Pageable pageable) {
    return this.ticketPort.getAll((root, query, criteriaBuilder) -> criteriaBuilder.conjunction(),
        pageable).map(this::convertEntityToModel);
  }
  ...
  public Ticket convertEntityToModel(TicketEntity entity) {
    ...
    Ticket ticket = Ticket.builder().ticketId(entity.getId().getTicketId()).title(entity.getTitle())
